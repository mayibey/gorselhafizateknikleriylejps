...
| 4 | ✅ | **Görsel TEK TİP STANDART kutu + tam görünsün + butonlar tam sığsın** | İterasyon: maxHeight→yan boşluk · tavansız→portre itiyor · kare+cover→üst/alt kırpılıyor (ekran hatasi 03). ✅ **Son: sabit DİKEY kutu (`aspectRatio:0.8`) + `contentFit:contain`** → her kart aynı ölçü, görselin TAMAMI görünür (kırpma yok). **akis.tsx:** Sesli Anlatım + Madde Metni ScrollView'dan çıkıp **alt pinli bloğa** (butonların hemen üstüne) taşındı → kontroller hep en altta sabit. Kutu oranına uymayan görselde ince kenar boşluğu (kremi mat). **İDEAL KALICI:** kaynak görselleri 4:5 (veya tek bir orana) üret → boşluk sıfırlanır (#6 içerik). |
