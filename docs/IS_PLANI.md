...
| 4 | ⬜ | **Görsel boyutu standart değil** → kart alanı kimi büyük kimi küçük → Sesli Anlatım + Madde Metni altta kalıp kaydırma gerekiyor | StudyCard görselin gerçek oranını uyguluyor (`6438ac8`). Sabit/`maxHeight` ile standardize → alt bölümler (TtsBar, Madde Metni) hep görünür. Layout işi; içerik-üretim oranıyla da ilgili. |
