...
| 4 | ✅ | **Görsel kart alanını tam doldurmalı** (önce: boyut tutarsız; sonra: yanlarda boşluk) | ⚠️ İlk deneme `maxHeight` tavanıydı → kareye yakın görseli sıkıştırıp **yan boşluk** yaptı (kullanıcı `ekranhatasi.jpg` ile bildirdi). ✅ Düzeltme: **tavan KALDIRILDI** → `aspectRatio:oran` + `width:100%` + contain → kutu oranı = görsel oranı → görsel **tam genişlik doldurur, boşluk yok**. Ödünleşme: çok uzun (portre) görselde TtsBar/Madde Metni scroll'da kalabilir (içerik "4 panelli kare" olduğundan çoğu sığar). |
