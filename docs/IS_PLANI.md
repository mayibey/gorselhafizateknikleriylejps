...
| 4 | ✅ | **Görsel TEK TİP STANDART + butonlar tam sığsın** | İterasyon: (1) maxHeight tavanı → yan boşluk (ekranhatasi.jpg). (2) tavan kaldırıldı → tam dolar ama portre görsel (m.3) kontrolleri itiyor (ekran hatasi 02). ✅ **Son: `study-card.tsx` görsel = sabit KARE alan (`aspectRatio:1`) + `contentFit:cover`.** Her kart aynı boyut, tam genişlik dolar (boşluk yok), Sesli Anlatım + Madde Metni hep sığar. Kare olmayan görsel merkezden kırpılır → tam hali **dokun-zoom**'da. Kullanılmayan `oran`/`useEffect` temizlendi. **İDEAL KALICI:** kaynak görselleri kare üret (kırpma kalmaz). |
