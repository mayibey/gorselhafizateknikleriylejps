...
| 9 | 🔧 | **Rütbe seçimi + içerik filtresi (Sb/Asb/Uzm.J/Uzm.Erb)** | ✅ Altyapı: `rutbe-store`/`rutbe-context`/`rutbe-secici`, onboarding 2-adım, guard, /rutbe-sec, Ayarlar girişi. ✅ Filtre mekanizması: `lib/rutbe-kapsam.ts > rutbeGorur` + Mevzuat'ta uygulanıyor. ⚠️ **VERİ KISMİ:** kaynak resmî Ek-1 (`jsps konuları.pdf`, masaüstü). Kilitli tek istisna **4678=law13 (uzmanlarda yok)**. **BEKLEYEN:** müşterek şüpheli satırlar (Sözleşmeli S/A Yön=16, E-imza, Türk Bayrağı) teyidi + **branş konularının rütbe matrisi** (PDF'in branş bölümleri, `-table -enc UTF-8` ile okunabiliyor). Eksikte güvenli yön = göster. |
