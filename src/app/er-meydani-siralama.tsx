    ...
    <Screen koyu={gece} title="Sıralama" onGeri={() => router.back()} headerAltinCizgi>
      ...
      {sampiyon ? (
        <View style={styles.sampiyonKart}>
          ...
          <View style={styles.sampiyonMetin}>
            <AppText variant="etiket" color={gece ? 'kartMetinIkincil' : 'solukMetin'} bold>GEÇEN HAFTANIN ŞAMPİYONU</AppText>
      ...
      {liste === null ? (
        ...
        <View style={styles.bos}>
          <MaterialCommunityIcons name="podium" size={44} color={gece ? Palette.kartMetinIkincil : Palette.solukMetin} />
        ...
        <View style={styles.listeSarma}>
          ...
          {!bendeListede && siram === null ? (
            <AppText variant="kucuk" color={gece ? 'kartMetinIkincil' : 'solukMetin'} style={styles.ipucu}>
    ...
    <View style={[styles.satir, satir.ben && styles.satirBen]}>
      <View style={[styles.siraNo, madalya && { backgroundColor: madalyaRenk }]}>
        {madalya ? (
          ...
          <AppText variant="kucuk" color={gece ? 'kartMetinAcik' : 'lacivert'} bold>{satir.sira}</AppText>
        )}
      </View>
      <View style={styles.satirOrta}>
        ...
        <AppText variant="etiket" color={gece ? 'kartMetinIkincil' : 'solukMetin'}>{satir.mac_sayisi} maç</AppText>
      </View>
      <AppText variant="altBaslik" color={gece ? 'altinParlak' : 'altinMetin'} bold>{satir.puan}</AppText>
      {!satir.ben ? (
        <Pressable
          hitSlop={10}
          ...
          <MaterialCommunityIcons name="dots-vertical" size={20} color={gece ? Palette.kartMetinIkincil : Palette.solukMetin} />
        </Pressable>
      ) : null}
    </View>
  );
