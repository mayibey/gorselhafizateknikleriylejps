...
export default function TelegramBaglanScreen() {
  ...
  return (
    <Screen title="Telegram'a Bağlan" onGeri={() => router.back()}>
      <AppText variant="govde" style={styles.aciklama}>
      ...
      {!hazir ? null : !kullanici ? (
        <View style={styles.kart}>
          <AppText variant="govde">Önce giriş yapmalısın komutan.</AppText>
          <Pressable style={styles.btn} onPress={() => router.push('/giris')}>
            <AppText variant="govde" color="beyaz" bold>Giriş Yap</AppText>
          </Pressable>
        </View>
      ) : (
        <View style={styles.kart}>
          <AppText variant="etiket" color="solukMetin" bold>BAĞLANMA KODUN</AppText>
          {yukleniyor ? (
            <ActivityIndicator color={Palette.altin} style={{ marginVertical: Spacing.three }} />
          ) : hata ? (
            <AppText variant="govde" color="kirmizi">Kod üretilemedi. İnternetini kontrol edip tekrar dene.</AppText>
          ) : (
            <AppText variant="dev" style={styles.kod}>{kod ?? '——————'}</AppText>
          )}

          <AppText variant="kucuk" color="solukMetin" style={styles.adim}>
            1) Aşağıdaki butonla Telegram'da botu aç (kod otomatik gider){'\n'}
            2) Açılmazsa botta şunu yaz: /baglan {kod ?? 'KOD'}
          </AppText>

          <Pressable style={styles.btn} onPress={telegramAc}>
          ...
          <Pressable style={styles.btnIkincil} onPress={uret} disabled={yukleniyor}>
            <AppText variant="kucuk" color="altinKoyu" bold>Yeni Kod Üret</AppText>
          </Pressable>
        </View>
      )}

      <AppText variant="kucuk" color="solukMetin" style={styles.not}>
