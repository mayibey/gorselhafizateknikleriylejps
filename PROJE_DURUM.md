...
- **Sıradaki BÜYÜK altyapı kararı:** (a) **Üyelik + ödeme** (gerçek user ID — filigranı/ilerlemeyi kişiye bağlar, paywall), (b) **Akıllı sistem KATMAN 3** ("eksikleri çalış" aksiyonu — zayıf kartları akışa besle), (c) **Sesli Nöbet v2** (arka plan/kilit ekranı — dev build). Paralel hat: içerik maratonu (her şeyi anlamlı kılar). Not: v2 ses + gerçek ses testi + arka plan = **development build** gerektiriyor (Expo Go ötesi).
