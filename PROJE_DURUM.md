...
- **Sırada: içerik maratonu** — TCK 49 görselini gerçek kartlara bağla (madde no + başlık + anlatım), sonra diğer müşterek + jandarma kanunlarının içeriği. (Şu an toplam 4 kart → hazırlık % paydası küçük; içerik artınca anlamlanır.) Ardından **Tatbikat ekranı** (quiz, şu an placeholder).
