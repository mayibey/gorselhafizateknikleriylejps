...
- **Akıllı öğrenme KATMAN 1 (veri toplama) BİTTİ (`7df2b3c`).** `kart_performans` tablosu + migration v4; çalışma (zor dahil tüm cevaplar) ve quiz (doğru/yanlış) cevapları LOG'lanıyor. SRS'ten bağımsız. DB şema sürümü **4**. Henüz UI yok. **Sırada KATMAN 2:** `lib/performans.ts` saf aggregator (geri besleme havuzu = çalışma'zor' ∪ quiz'yanlis', recency ağırlıklı) + Karargah/Sicil "eksik kapatma" gösterimi + opsiyonel "eksikleri çalış" akışı.
...
- **Ses altyapısı FAZ 2 BİTTİ (`c6b15f8`).** expo-audio ile tek-kart "Dinle" (oynat/duraklat, auto-play yok); ses registry+codegen boş hazır; sessiz kartlar "Sesli anlatım yakında". Ses dosyaları (içerik) + "Sesli Nöbet" (arka arkaya kuyruk, kilit ekranı, arka plan modu) **sonraki faz**. AudioBar artık akışta görünür (gizli değil).
- **Sırada (içerik): içerik maratonu** — TCK 49 görselini gerçek kartlara bağla (madde no + başlık + anlatım), sonra diğer müşterek + jandarma kanunlarının içeriği. (Şu an toplam 4 kart → hazırlık % paydası küçük + quiz havuzu dar; içerik artınca hem istatistik hem quiz hem performans analizi anlamlanır.)
- İsteğe bağlı sonraki istatistik işi (gerekirse): "bugün çalışılan kart" için `study_days`'e `(gun, adet)` kolonu (sonraki migration v5); streak için ŞART değil.
- **Sıradaki BÜYÜK altyapı kararı (ikisinden biri):** (a) **Üyelik + ödeme** (gerçek user ID — filigranı/ilerlemeyi kişiye bağlar, paywall), veya (b) **Akıllı sistem KATMAN 2** (biriken `kart_performans` üzerinden geri besleme havuzu + "eksik kapatma"). Üçüncü paralel hat: içerik maratonu (her şeyi anlamlı kılar).
