...
const tik=async(x,y)=>{await g('Input.dispatchMouseEvent',{type:'mouseMoved',x,y});await g('Input.dispatchMouseEvent',{type:'mousePressed',x,y,button:'left',clickCount:1});await g('Input.dispatchMouseEvent',{type:'mouseReleased',x,y,button:'left',clickCount:1});await bekle(500);};
const yaz=async(txt)=>{await g('Input.insertText',{text:txt});await bekle(400);};
// Urun kimligi
await tik(960,512); await yaz('musterek_aylik');
// Ad
await tik(960,611); await yaz('Tam Erişim Aylık');
await bekle(600);
const s=await g('Page.captureScreenshot',{format:'png'});fs.writeFileSync('scratchpad/pc.png',Buffer.from(s.data,'base64'));
console.log('form dolduruldu, pc.png');
process.exit(0);
