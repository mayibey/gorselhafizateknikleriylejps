...
const ch = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe',
  ['--headless=new','--disable-gpu','--no-first-run',`--remote-debugging-port=${PORT}`,`--user-data-dir=${KOK}/chrome-bf`,'about:blank'],{stdio:'ignore'});
...
async function test(c, url){
  await c.g('Page.navigate',{url});await bekle(2500);
  ...
  // barBas'a observer kur, 4 harf tuşuna bas, mutation say
  const r=await evl(c,`(function(){
    var bb=document.querySelector('#barBas'); if(!bb)return 'barBas YOK';
    var say=0; var obs=new MutationObserver(function(ms){say+=ms.length;});
    obs.observe(bb,{childList:true,characterData:true,subtree:true});
    var tusar=[...document.querySelectorAll('.tus')].filter(t=>/^[A-ZÇĞİÖŞÜ]$/.test((t.dataset.t||t.textContent||'').trim()));
    var basildi=0;
    for(var i=0;i<4 && i<tusar.length;i++){ tusar[i].click(); basildi++; }
    var res={baslik:bb.textContent, tusVar:tusar.length, basildi:basildi, mutation:say};
    obs.disconnect(); return JSON.stringify(res);
  })()`);
  return r;
}
try{
  ...
  console.log('GUARD-SIZ (canli-son):', await test(c,`file:///${KOK}/canli-son.html`));
  console.log('GUARD-LI  (canli-fix):', await test(c,`file:///${KOK}/canli-fix.html`));
}finally{ch.kill();}
