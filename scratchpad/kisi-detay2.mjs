...
console.log('log kolonlari:', (await sql("select column_name from information_schema.columns where table_name='icerik_erisim_log' order by ordinal_position")).map(r=>r.column_name).join(', '));
console.log('Onun kaydi   :', await sql(`select count(*) n, max(zaman)::date son from icerik_erisim_log where kullanici_id='${ID}'`).catch(e=>String(e).slice(0,200)));
