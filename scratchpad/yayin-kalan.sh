...
MESAJ="Bozuk soru duzeltildi (2803 madalyalar) - Mustafa Ozkan bildirdi"
for V in 1.0.41 1.0.40; do
  ...
  EAS_NO_VCS=1 TMPDIR=D:/easbuild-tmp NODE_OPTIONS=--max-old-space-size=8192 \
    npx eas update --branch production --message "$MESAJ ($V)" --non-interactive 2>&1 | grep -E "Runtime version|Update group ID|error" || true
done
