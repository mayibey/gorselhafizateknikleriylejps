...
console.log('AYARLAR:', await sql("select anahtar,deger from uygulama_ayar where anahtar in ('zorunlu_min_surum','anlik_guncelleme_herkes')"));
console.log('SÜRÜM TABLOLARI:', await sql("select table_name from information_schema.tables where table_schema='public' and (table_name ilike '%surum%' or table_name ilike '%cihaz%' or table_name ilike '%oturum%')"));
