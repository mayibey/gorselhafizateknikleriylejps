...
const KOK='D:/GorselHafizaTeknikleriyleJSPS/scratchpad';const PORT=9474;const bekle=ms=>new Promise(r=>setTimeout(r,ms));
const ch=spawn('C:/Program Files/Google/Chrome/Application/chrome.exe',['--headless=new','--disable-gpu','--no-first-run',`--remote-debugging-port=${PORT}`,`--user-data-dir=${KOK}/chrome-at`,'about:blank'],{stdio:'ignore'});
...
for(const [id,ac] of [['terazi',"haritaAc('terazi')"],['hangi',"haritaAc('hangi')"]]){
  await evl(c,`try{${ac}}catch(e){}`);await bekle(800);
  ...
  const s=await c.g('Page.captureScreenshot',{format:'png'});if(s?.data)fs.writeFileSync(`${KOK}/atest-${id}.png`,Buffer.from(s.data,'base64'));
  const sik=await evl(c,"(function(){var els=[...document.querySelectorAll('#govde button, #govde .sik, #govde .mSat, #govde [class*=Sat], #govde .hc')].filter(e=>e.getBoundingClientRect().width>200);var e=els[0];return e?JSON.stringify({cls:e.className.slice(0,25),bg:getComputedStyle(e).backgroundColor}):'yok';})()");
  console.log(id,sik);
}}finally{ch.kill();}
