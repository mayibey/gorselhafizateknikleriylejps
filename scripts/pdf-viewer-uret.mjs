...
// RN çağırır: PDF base64 + kayıtlı notlar (sayfa->stroke dizisi) + dosya yolu.
...
function sayfaCiz(n){
  return pdfDoc.getPage(n).then(function(page){
    ...
    cizimKur(oc, n);
    if (notlar[n]) cizStroke(oc, notlar[n]);
    return page.render({canvasContext:c.getContext('2d'), viewport:vp}).promise;
  });
}

// Overlay'e kayıtlı stroke'ları çiz.
function cizStroke(oc, liste){
  ...
  liste.forEach(function(s){
    ...
    ctx.lineWidth = s.t==='fosfor'?18:(s.t==='silgi'?24:3); ctx.lineCap='round'; ctx.lineJoin='round';
...
// Bir sayfada çizim (parmakla) + kaydetme.
function cizimKur(oc, n){
  ...
  function bas(e){ if(arac==='pan') return; e.preventDefault(); ciziyor=true; nokta=[xy(e)]; }
  function hareket(e){ if(!ciziyor) return; e.preventDefault(); nokta.push(xy(e));
    ...
    ctx.lineWidth=arac==='fosfor'?18:(arac==='silgi'?24:3); ctx.lineCap='round'; ctx.lineJoin='round';
    ...
    ctx.globalAlpha=1; }
  function birak(){ if(!ciziyor) return; ciziyor=false; if(nokta.length>1){ notlar[n].push({t:arac,c:renk,p:nokta}); kaydet(n); } }
...
window.addEventListener('DOMContentLoaded', function(){
  ...
  [['pan','↕','Gez'],['kalem','✏️','Kalem'],['fosfor','🖍️','Fosfor'],['silgi','🧽','Silgi']].forEach(function(a){
  ...
  ['#e53935','#1e88e5','#43a047','#fdd835','#000000'].forEach(function(c){
...
<div id="pages"></div>
<div id="cubuk"><div id="arac" style="display:flex;gap:8px"></div><div id="renkler"></div></div>
