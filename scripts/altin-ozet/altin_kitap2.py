...
import re, io, os, html as H, subprocess, fitz, sys, datetime
...
SADECE_MUSTEREK = len(sys.argv) > 1 and sys.argv[1] == 'musterek'
OUT = ('C:/Users/GIGABYTE/OneDrive/Desktop/JSPS 2026 - ALTIN OZET (SADECE MUSTEREK).pdf' if SADECE_MUSTEREK
       else 'C:/Users/GIGABYTE/OneDrive/Desktop/JSPS 2026 - ALTIN OZET (Musterek + MEBS).pdf')
...
def nokta_html(l):
    ...
    ham = re.sub(r'^\s*[-*•]\s+', '', l)
    cikmis = bool(re.match(r'^★\s*(çıkmış|2024)', ham))
    etiket = ''
    m = re.match(r'^★\s*(çıkmış[^\s*]*|2024)\s*(\([^)]*\))?\s*', ham)
    if m:
        etiket = '★ 2024 SINAVI' if '2024' in m.group(1) else '★ ÇIKMIŞ'
        ham = ham[m.end():]
    # segmentlere ayır
...
bolumler = [('giris', 'Sınavı Yapan Kurumun Soru Mantığı', 'Bu yıl soruları ATA-AÖF yazıyor. Kurumun üslubu ve eski JSPS kitapçıklarının kalıpları.'),
            ...
            ('ozet_D', 'MEBS Branş Mevzuatı', 'Elektronik Haberleşme · Yetkilendirme · Taşınır Mal · Harcama Belgeleri · Kriptolu Haberleşme · Telsiz · 2019/12 · 2024/7 · Güvenlik Rehberi')]
if SADECE_MUSTEREK: bolumler = [b for b in bolumler if b[0] != 'ozet_D']
...
b {{ color: #0B1F3A; }}
.kapak {{ height: 252mm; display: flex; flex-direction: column; justify-content: space-between; background: #0B1F3A; color: #F7F3EA; border-radius: 3mm; page-break-after: always; padding: 22mm 18mm; }}
...
<div class="kapak">
 <div><div class="ust">JANDARMA VE SAHİL GÜVENLİK PERSONELİ SINAVI · 2026{" · MÜŞTEREK" if SADECE_MUSTEREK else ""}</div>
  ...
  <h2>{"Müşterek Mevzuat — 25 Kanun ve Yönetmelik" if SADECE_MUSTEREK else "Müşterek Mevzuat (25) &nbsp;+&nbsp; MEBS Branş Mevzuatı (9)"}</h2>
  <div class="kutular">
   <div class="kt"><div class="s">{"808" if SADECE_MUSTEREK else "1.093"}</div><div class="a">altın nokta<br>hüküm + tuzağı</div></div>
   <div class="kt"><div class="s">{"375" if SADECE_MUSTEREK else "512"}</div><div class="a">çıkmış sınavda<br>fiilen sorulmuş</div></div>
   <div class="kt"><div class="s">{"25" if SADECE_MUSTEREK else "34"}</div><div class="a">{"müşterek mevzuat<br>emir kapsamıyla sınırlı" if SADECE_MUSTEREK else "mevzuat<br>emir kapsamıyla sınırlı"}</div></div>
   <div class="kt"><div class="s">2.336</div><div class="a">incelenen<br>çıkmış soru</div></div>
  </div></div>
 <div class="alt">Bu kitap üç kaynaktan üretildi: <b style="color:#F3E7C1">bu yılın emri (Ek-1, 21 Mayıs 2026)</b> ile sınırlı resmî madde metinleri ·
 26 çıkmış JSPS kitapçığından 2.336 soru · {"19 Ekim 2024 tarihli çıkmış kitapçığın kaynağı doğrulanmış müşterek soruları." if SADECE_MUSTEREK else "19 Ekim 2024 Subay MEBS kitapçığının kaynağı doğrulanmış 100 sorusu."}<br>
 Sınavı bu yıl Atatürk Üniversitesi ATA-AÖF yapıyor; kurumun kendi sınavlarından 120 soru incelenerek soru üslubu çıkarıldı.<br>
 Mevzu JSPS · {bugun}</div>
</div>
<div class="ic"><h1>İçindekiler</h1>{ic_html}</div>
{govde}
</body></html>'''
hp = S + ('_kitap_m.html' if SADECE_MUSTEREK else '_kitap2.html'); io.open(hp, 'w', encoding='utf-8').write(HTML)
print('sade sürüm kullanılan bölüm:', sade_sayisi, '/ 4')
subprocess.run([CHROME, '--headless=new', '--disable-gpu', '--no-pdf-header-footer', f'--print-to-pdf={S}' + ('_kitap_m.pdf' if SADECE_MUSTEREK else '_kitap2.pdf'), 'file:///' + hp], capture_output=True, timeout=600)
d = fitz.open(S + ('_kitap_m.pdf' if SADECE_MUSTEREK else '_kitap2.pdf'))
toc = []; cari = ''
...
for n in (4, 12, 40, 100):
    if n < len(d): d[n].get_pixmap(dpi=58).save(S + ('_m_' if SADECE_MUSTEREK else '_v2_') + f'{n}.png')
